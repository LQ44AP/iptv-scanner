...
// 常量定义
#define MAX_POOL_SIZE 5000          // 保留（实际上不再使用）
...
// 结构体：记录唯一的 IP 和端口组合（仅用于哈希表去重）
typedef struct {
    uint32_t ip;
    uint16_t port;
    uint8_t proto_type;  // 0=RTP, 1=TS
} DiscoveredNode;        // 实际未使用，保留结构体以备后续扩展
...
static int g_channel_count = 1;          // M3U 频道序号
...
// 当前扫描的多播 IP（用于在 handler 中精确过滤）
static uint32_t g_current_mcast_ip = 0;
...
int is_duplicate(uint32_t ip, uint16_t port) {
    ...
    if (!new_node) {
        log_message("错误：哈希节点内存分配失败，该频道将被视为重复以避免输出混乱\n");
        return 1;   // 内存不足时视为重复，避免错误输出
    }
    new_node->key = key;
...
void packet_handler(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
    ...
    // 精确过滤：只处理当前正在扫描的多播组（避免混杂模式下收到其他组播流量）
    if (dest_ip != g_current_mcast_ip) {
        return;
    }
    ...
    if (inet_ntop(AF_INET, &dest_ip, ip_str, sizeof(ip_str)) == NULL) {
        snprintf(ip_str, sizeof(ip_str), "无效IP");
    }
...
void scan_single_ip(pcap_t *handle, const char *prefix, int last_byte) {
    ...
    // 绑定到任意端口，使套接字能够发送 IGMP 报告并接收组播流量
    struct sockaddr_in local;
    memset(&local, 0, sizeof(local));
    local.sin_family = AF_INET;
    local.sin_port = htons(0);          // 系统分配任意端口
    local.sin_addr.s_addr = htonl(INADDR_ANY);
    if (bind(s, (struct sockaddr*)&local, sizeof(local)) < 0) {
        log_message("绑定套接字失败: %s\n", strerror(errno));
        close(s);
        return;
    }
    ...
    // 设置当前扫描 IP，供 packet_handler 过滤
    g_current_mcast_ip = mreq.imr_multiaddr.s_addr;

    if (setsockopt(s, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) != 0) {
        log_message("加入多播组 %s 失败: %s\n", mcast_ip, strerror(errno));
        close(s);
        return;
    }
...
int validate_arguments(int argc, char *argv[]) {
    if (argc < 5) {
        printf("\nIPTV 严格去重探测扫描器 - 改进版（支持VLAN/BPF过滤）\n");
...
int main(int argc, char *argv[]) {
    ...
    fprintf(fp_out, "# Generated by IPTV Scanner (improved) at %s\n", get_current_time());
    ...
    // 改进：增大 snaplen 以捕获完整数据包（尤其带 VLAN 标签时）
    pcap_set_snaplen(handle, 65535);
    pcap_set_promisc(handle, 0);
    pcap_set_timeout(handle, 1000);   // 1秒超时，用于阻塞式读取
    ...
    if (pcap_setnonblock(handle, 0, errbuf) < 0) {
        log_message("设置阻塞模式警告: %s\n", errbuf);
    }

    // 编译 BPF 过滤器（仅过滤 UDP 多播）
